...
items.push(async ({ dir }) => {
    ...
    if (await fs.exists(path.join(dir, 'www', 'index.html')) === false) {
        return;
    }
    ...
    if (!/<meta\s+http-equiv="Content-Type"\s+content="text\/html;\s*charset=UTF-8"\s*\/>/
        .test(indexHtml)) {
            console.error('A cordova project index.html file should have a Content-Type meta field. '
